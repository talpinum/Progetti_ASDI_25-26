...
--- num_fronti_per_TX = 50 * 10^6 / 9600 = 5208 (il conteggio Ã¨ usato per "generare un CLK TX")
--- lato ricevitore dovrÃ² usare un clock 16 volte piÃ¹ veloce di CLK TX:
--- num_fronti_per_RX = num_fronti_per_TX/16 = 326
--- questo codice genera un clock in ricezione (rCLK) con duty cycle 50% quindi dovrÃ  essere
--- alto per 163 fronti e basso per 163 fronti => il valore di conteggio che consente di generare
--- questo rCLK Ã¨ baudDivide
--- NOTA: in questo codice il tCLK Ã¨ generato a partire da rCLK
	...
	signal rClkDiv :  std_logic_vector(3 downto 0)	:= "0000";		    --usato per tClk: il MSB di questa stringa Ã¨ usato come tClk
...
--          appena si riconosce lo start bit sia per campionare i successivi bit ogni 16 colpi di rClk
		...
		begin										--un contatore modulo 16 (con possibilitÃ  di reset)
			if rClk = '1' and rClk'Event then
				if ctRst = '1' then                 --ctRst diventa 1 la prima volta appena viene rilevato lo start bit e si passa
													...
													--si entra nello stato strGetData perchÃ¨ il ricevitore si Ã¨ posizionato a centro bit			
													
													--infine ctRst diventa 1 quando ctr conta 16 colpi di clock perchÃ¨ vuol dire che un
													--nuovo bit Ã¨ stato acquisito
...
--          cioÃ¨ quando il ricevitore si Ã¨ posizionato a centro bit dopo aver ricevuto lo start e inizia a campionare i bit del frame
...
--  -----   quando la CPU legge il dato dal buffer pone RD=1 e questo farÃ  abbassare RDA
	process (rClk, RST, RD, CE)
		begin
			if RD = '1' or RST = '1' then
				...
				if CE = '1' then      
					FE <= frameError;
					OE <= RDA;   --se RDA era ancora alto vuol dire che nessuno ha ancora letto il dato e quindi si avrÃ  averrun
		...
		begin   	   
			...
			case sttCur is
				...
				when sttIdle =>
					...
					if TBE = '1' then  --se la trasmissione del frame precedente Ã¨ terminata torna in Idle
						sttNext <= sttIdle;
					else
						sttNext <= sttTransfer; --appena TBE si azzera (cioÃ¨ appena WR=1) inizia a trasmettere
					end if;	
				...
				when sttShift =>
					...
					load <= '0';
					tClkRST <= '0';
					if tfCtr = "1100" then --la trasmissione torna in Idle quando il contatore dei bit inviati Ã¨ tfCtr=12
										   --in realtÃ  i bit del frame sono 11 ma bisogna attendere almeno un ciclo di Clock
...
--  ----------------------------------------------------------------------------------------
-- il segnale TBE Ã¨ inizialmente alto, e diventa basso appena si avvia una nuova trasmissione
-- (quando viene dato il segnale di WRITE) per poi tornare alto quando il caricamente Ã¨ stato terminato.
-- Nel codice, il segnale TBE Ã¨ connesso al segnale load di caricamento dello shift register e a WR:
